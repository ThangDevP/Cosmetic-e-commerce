...
async function fetchDataAndPopulateTable() {
    try {
      const response = await fetch('http://localhost:3000/api/orders?_expand=user');
      ...
      for (const order of data) {
        ...
        for (const item of order.itemsDetails) {
          ...
          const productResponse = await fetch(`http://localhost:3000/api/products/${item.productId}`);
