...
var useruserId = localStorage.getItem("userID")
async function addToCart(productId) {
  try {
    // Bước 1: Kiểm tra xem người dùng đã đăng nhập chưa

    // Bước 2: Tìm giỏ hàng của người dùng
    const response = await fetch(`/api/carts?userId=${useruserId}`);
    if (!response.ok) {
    ...
    if (cartItems.length > 0) {
      ...
      const updateCartItemResponse = await fetch(`/api/cartItems/${cartItemId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
